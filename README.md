...
uv run python scripts/market_data_query.py daily-pack --cutoff-at 2026-07-29T13:00:00Z --persistence none --pretty
...
- `scripts/market_data_query.py daily-pack` 是无需启动 HTTP 服务的一次性中美日报
  行情数据包；固定 `persistence=none`，不读写 SQLite 或 scheduler state
