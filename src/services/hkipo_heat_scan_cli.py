...
from .hkipo_heat_scan_service import HkIpoHeatScanService, normalize_scan_payload

DEFAULT_TIMEZONE = "Asia/Shanghai"


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Scan readonly HK IPO heat evidence")
...
def main(
    ...
    *,
    ...
    service: HkIpoHeatScanService | None = None,
) -> int:
    parser = build_parser()
    ...
    try:
        ...
        scanner = service or HkIpoHeatScanService()
        raw_payload = scanner.scan(
            report_date=report_date,
            ipos=ipos,
            include_closed=bool(args.include_closed),
        )
        payload = normalize_scan_payload(raw_payload, report_date)
