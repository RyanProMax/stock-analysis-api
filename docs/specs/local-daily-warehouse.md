...
- `scripts/alpha_universe_seed_status.py` 是只读状态检查入口，用于对 seed 内 symbols 做精确覆盖检查并输出 missing / incomplete / stale；`start_date` 会解析为本地仓在该日期及之后的首个已观测交易日，避免把非交易日起点误判为缺口；`sync_plan.command_args` 只提供补库建议，不执行补库、不写同步状态、不触发外部 provider。
